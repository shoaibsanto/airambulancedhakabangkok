...
function metaContent(html, attr, value) {
  ...
  // matches <meta name="x" content="..."> AND <meta content="..." name="x">
  // (attribute order in the source HTML is not guaranteed — some pages emit
  // content= before name=/property=, which the original attr-before-content
  // -only regex silently failed to match, dropping the description/robots/OG
  // tags entirely on every affected page).
  ...
  const m = html.match(reAttrFirst) || html.match(reContentFirst);
  return m ? decodeEntities(m[1]) : undefined;
}

function linkHref(html, rel) {
  const reRelFirst = new RegExp(`<link[^>]*rel=["']${rel}["'][^>]*href=["']([^"']*)["']`, "i");
  const reHrefFirst = new RegExp(`<link[^>]*href=["']([^"']*)["'][^>]*rel=["']${rel}["']`, "i");
  const m = html.match(reRelFirst) || html.match(reHrefFirst);
